...
describe("the kinds a section can be", () => {
    ...
    it("finds the blurb from an index that never named one", () => {
        ...
        expect(
            generatedCell([markdown("a"), recap("a.author")], -1, "recap"),
        ).toBe(1);
    });
    ...
    it("counts a box holding only spaces as unfilled", () => {
        expect(unfilledFields(recap("   ")).map((f) => f.name)).toEqual([
            "documents",
        ]);
    });
    ...
    it("reads the documents a story so far is written from", () => {
        expect(
            documentsOf(recap("parts/part_1.author, parts/part_2.author")),
        ).toEqual(["parts/part_1.author", "parts/part_2.author"]);
    });
    ...
    it("gives the story so far the one field it needs and no title", () => {
        ...
        expect(fieldsOf("recap").map((field) => field.name)).toEqual([
            "documents",
        ]);
    ...
    it("gives a part a name, and a box saying whether the book prints it", () => {
        ...
        expect(fieldsOf("part").map((f) => f.toggle)).toEqual([
            undefined,
            true,
        ]);
...
describe("compile — the sections that are built rather than written", () => {
    ...
    it("empties the listing when there are no chapters left", () => {
        const compiled = compile([
            { kind: "contents", source: "1. Gone", attrs: {} },
        ]);
...
describe("isStale — the freshness a notebook shows as its execution count", () => {
    ...
    it("is never stale for a cell nobody builds", () => {
        ...
        expect(isStale([{ kind: "epigraph", source: "", attrs: {} }], 0)).toBe(
            false,
        );
    });
...
describe("placeOf — where in the book a cell stands", () => {
    ...
    it("puts a chapter heading in its own chapter", () => {
        expect(placeOf([chapter("One")], 0)).toEqual({
            part: null,
            chapter: "One",
        });
    });
...
describe("wordsIn — what the story weighs", () => {
    ...
    it("adds up every markdown section in the document", () => {
        expect(
            wordsIn([markdown("one two"), markdown("three four five")]),
        ).toBe(5);
    });
    ...
    it("does not count the book\u2019s own pages", () => {
        const cells: Cell[] = [
            ...
            {
                kind: "cover",
                source: "![Cover](art/cover.jpg)",
                attrs: { src: "art/cover.jpg" },
            },
            {
                kind: "disclaimer",
                source: "All persons are fictitious.",
                attrs: {},
            },
            { kind: "about", source: "She lives by the sea.", attrs: {} },
    ...
    it("counts nothing in a kind it has never heard of", () => {
        const cells: Cell[] = [
            { kind: "epigraph", source: "a b c", attrs: {} },
        ];
...
describe("toMarkdown — writing a plain manuscript out", () => {
    ...
    it("writes prose as itself", () => {
        expect(toMarkdown([markdown("It began badly.")])).toBe(
            "It began badly.\n",
        );
    });
    ...
    it("separates sections by a blank line", () => {
        expect(
            toMarkdown([chapter("One"), markdown("a"), chapter("Two")]),
        ).toBe("### One\n\na\n\n### Two\n");
    });
            ...
            toMarkdown([
                {
                    kind: "about",
                    source: "",
                    attrs: { substack: "https://s.example" },
                },
            ]),
    ...
    it("keeps a note that says `-->` inside its comment", () => {
        ...
        expect(
            toMarkdown([
                { kind: "note", source: "Anna --> the tower.", attrs: {} },
            ]),
        ).toBe("<!--\nAnna --&gt; the tower.\n-->\n");
    });
...
describe("folding a section away", () => {
    ...
    it("folds every section, and unfolds every one", () => {
        ...
        expect(foldEvery(foldEvery(cells, true), false).some(isFolded)).toBe(
            false,
        );
    });
});
