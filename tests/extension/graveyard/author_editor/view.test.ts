...
describe("the toolbar", () => {
    ...
    it("counts the chapters for the status", async () => {
        ...
        expect(document.getElementById("doc-status")!.textContent).toContain(
            "2 chapters",
        );
    });
...
describe("the word count", () => {
    ...
    it("says what the story weighs, beside what it is made of", async () => {
        await mount([
            chapter("One"),
            markdown("The lantern had gone out again."),
        ]);
...
describe("a section as a panel — header, body, footer", () => {
    it("gives every section a header saying what it is", async () => {
        ...
        expect(
            shown().map(
                (c) => c.querySelector(".cell-head .cell-kind")!.textContent,
            ),
    ...
    it("puts the name at the left and the kind and buttons after it", async () => {
        ...
        expect(
            [...head.children].map((child) => child.className.split(" ")[0]),
        ).toEqual(["cell-title", "cell-kind", "actions"]);
    });
    ...
    it("puts the name in the header and the rest of the facts in the body", async () => {
        ...
        expect(
            cell.querySelectorAll(".cell-body .cell-field-row"),
        ).toHaveLength(6);
    });
    ...
    it("puts what the section comes to in the footer, and nowhere else", async () => {
        ...
        expect(
            shown()[0].querySelector(".cell-foot .cell-words")!.textContent,
        ).toBe("3 words");
    ...
    it("is put away with the body when the section is folded", async () => {
        ...
        shown()[0]
            .querySelector<HTMLElement>(
                '.actions [aria-label="Fold this section away"]',
            )!
            .click();
        send(lastCells());
        expect(weighs(0)).toBeNull();
        expect(
            shown()[0].querySelector(".cell-head .cell-title"),
        ).not.toBeNull();
    });
});
...
describe("drawing cells", () => {
    ...
    it("gives a chapter its title field and no prose editor", async () => {
        await mount([chapter("One")]);
        const cell = shown()[0];
        expect(cell.querySelector<HTMLInputElement>(".cell-title")!.value).toBe(
            "One",
        );
    ...
    it("gives a part a box saying whether the book prints a page for it", async () => {
        ...
        expect(
            cell.querySelector<HTMLInputElement>(".cell-toggle")!.checked,
        ).toBe(true);
        // On the row every other field is written on, named in the same column.
        const row = cell.querySelector(".cell-field-row")!;
        expect(row.querySelector(".cell-field-label")!.textContent).toBe(
            "Printed",
        );
    ...
    it("says what kind every cell is, which is what the stylesheet draws it by", async () => {
        ...
        expect(
            shown().map((cell) => (cell as HTMLElement).dataset.kind),
        ).toEqual(["markdown", "part", "chapter"]);
    });
    ...
    it("hints at the date format in the empty box", async () => {
        await mount([
            { kind: "title-page", source: "", attrs: { title: "T" } },
        ]);
        const boxes = [
            ...shown()[0].querySelectorAll<HTMLInputElement>(".cell-field"),
        ];
    ...
    it("gives a disclaimer both a heading and a prose body", async () => {
        ...
        expect(cell.querySelector(".rendered")!.textContent).toContain(
            "Careful.",
        );
    });
    ...
    it("keeps the box a folded section is named in, since the header is its own", async () => {
        await mount([
            ...
            {
                kind: "markdown",
                source: "The lantern had gone out.",
                attrs: { folded: "true" },
            },
        ]);
        expect(
            shown()[0].querySelector<HTMLInputElement>(
                ".cell-head .cell-title",
            )!.value,
        ).toBe("One");
        // A section with no name of its own is its kind, folded or open.
        expect(shown()[1].querySelector(".cell-head")!.textContent).toBe(
            "Markdown",
        );
    });
    ...
    it("unfolds it again from the same place", async () => {
        ...
        shown()[0]
            .querySelector<HTMLElement>(
                '.actions [aria-label="Unfold this section"]',
            )!
            .click();
    ...
    it("renders prose as markdown rather than as its source", async () => {
        ...
        expect(shown()[0].querySelector(".rendered")!.innerHTML).toContain(
            "<h1>",
        );
    });
...
describe("opening a cell", () => {
    ...
    it("carries the open cell with it when the cell is moved", async () => {
        ...
        shown()[1]
            ...
            .dispatchEvent(new MouseEvent("dblclick", { bubbles: true }));
        (
            shown()[1].querySelector('[aria-label="Move up"]') as HTMLElement
        ).click();
        echo();
...
describe("leaving a cell", () => {
    ...
    function press(
        ...
        box.dispatchEvent(
            new KeyboardEvent("keydown", { key, bubbles: true, ...held }),
        );
    }
    ...
    it("gives up the other cursors before it gives up the cell", async () => {
        ...
        expect(
            document.querySelectorAll(".source-cursors .at"),
        ).not.toHaveLength(0);

        press(box, "Escape");
        expect(shown()[0].querySelector("textarea")).toBe(box);
        expect(document.querySelectorAll(".source-cursors .at")).toHaveLength(
            0,
        );
    });
    ...
    it("writes down the cell being left when another one is opened", async () => {
        ...
        expect(shown()[0].querySelector("textarea")).toBeNull();
        expect(shown()[1].querySelector("textarea")).not.toBeNull();
        expect(lastCells().map((cell) => cell.source)).toEqual([
            "a and more",
            "b",
        ]);
    });
...
describe("changing the document", () => {
    ...
    it("adds a note from the bar, beside the two kinds it is written among", async () => {
        ...
        await mount([markdown("a")]);
        const labels = [
            ...bars()[0].querySelectorAll<HTMLElement>(".insert"),
    ...
    it("deletes the cell the button belongs to", async () => {
        await mount([markdown("a"), markdown("b")]);
        shown()[0]
            .querySelector<HTMLElement>(".actions .icon:last-child")!
            .click();
    ...
    it("moves a cell down", async () => {
        await mount([markdown("a"), markdown("b")]);
        shown()[0]
            .querySelector<HTMLElement>('.actions [aria-label="Move down"]')!
            .click();
    ...
    it("draws the move once the document comes back", async () => {
        ...
        send(lastCells());
        expect(
            shown().map((c) => c.querySelector(".rendered")!.textContent),
        ).toEqual(["b", "a"]);
    });
    ...
    it("records a chapter title as the author types it", async () => {
        await mount([chapter("One")]);
        const title =
            shown()[0].querySelector<HTMLInputElement>(".cell-title")!;
        title.value = "Renamed";
    ...
    it("writes the attribute when the author unticks the box, and only then", async () => {
        ...
        send(lastCells());
        const back =
            shown()[0].querySelector<HTMLInputElement>(".cell-toggle")!;
        back.checked = true;
...
describe("the story so far", () => {
    ...
    it("carries its own name, the documents box and the writing in the body", async () => {
        ...
        expect(
            cell.querySelector(".cell-body .rendered")!.textContent,
        ).toContain("She has lost her name.");
    });

    it("hints at how the documents are written in the empty box", async () => {
        await mount([recap()]);
        expect(
            shown()[0].querySelector<HTMLInputElement>(".cell-field")!
                .placeholder,
    ...
    it("folds the documents away with the writing", async () => {
        ...
        const cell = shown()[0];
        expect(cell.querySelector(".cell-name")!.textContent).toBe(
            "The Story So Far",
        );
        expect(cell.querySelector(".cell-body")).toBeNull();
    ...
    it("draws an unfilled parameter as a box rather than as a caption", async () => {
        ...
        expect(
            shown()[0]
                .querySelector(".cell-field")!
                .classList.contains("needed"),
        ).toBe(true);
        expect(
            shown()[1]
                .querySelector(".cell-field")!
...
describe("the menus", () => {
    it("lists only the kinds the bar has no button for", async () => {
        await mount([markdown("a")]);
        document
            .querySelector<HTMLElement>(".insert-bar .insert.icon-only")!
            .click();
...
describe("a cell the server is writing", () => {
    ...
    it("turns the run button into the button that stops it", async () => {
        ...
        writes(0);
        expect(
            shown()[0].querySelector(".run .codicon-primitive-square"),
        ).not.toBeNull();
    ...
    it("says only that it is writing until it knows how long the book is", async () => {
        ...
        writes(0, 0, 0);
        expect(
            shown()[0].querySelector<HTMLElement>(".writing-fill")!.style.width,
        ).toBe("0%");
        expect(shown()[0].querySelector(".writing-said")!.textContent).toBe(
            "Writing…",
        );
    ...
    it("takes the bar and the stop button with it when the job moves on", async () => {
        ...
        expect(shown()[0].querySelector(".writing")).toBeNull();
        expect(
            shown()[1].querySelector(".run .codicon-primitive-square"),
        ).not.toBeNull();
        expect(shown()[1].querySelector(".writing")).not.toBeNull();
    });
...
describe("a pass over the whole document", () => {
    ...
    it("says only that it is working until it knows how long the book is", async () => {
        ...
        styles(true, 0, 0);
        expect(
            (document.getElementById("job-fill") as HTMLElement).style.width,
        ).toBe("0%");
        expect(document.getElementById("job-said")!.textContent).toBe(
    ...
    it("says why nothing is moving when the model has told it to wait", async () => {
        ...
        styles(
            ...
            0,
            5,
            "gemini-3.7-flash is rate limited — waiting 47s (try 2 of 6)",
        );
        expect(document.getElementById("job-said")!.textContent).toBe(
...
describe("dividing a section", () => {
    ...
    it("draws a line over a section of prose the pointer is on", async () => {
        ...
        expect((shown()[0].querySelector(".seam") as HTMLElement).hidden).toBe(
            false,
        );
    });
    ...
    it("offers no join where the section above is another kind", async () => {
        ...
        shown()[1].dispatchEvent(new MouseEvent("mousemove"));
        expect((shown()[1].querySelector(".seam") as HTMLElement).hidden).toBe(
            true,
        );
    });
});
